...
pub(crate) async fn capabilities(command: crate::CapCommand) {
    ...
    match command {
        crate::CapCommand::Grant {
            ...
            {
                Ok(cap) => {
                    println!("Granted {} on {} to {}", cap.id, cap.resource.canonical(), cap.subject);
                    if let Some(exp) = cap.expires_at {
                        println!("Expires: {exp}");
                    }
                    // Print the transferable bundle so it can be handed
                    // to the subject over any channel.
                    let encoded = BASE64.encode(bincode::serialize(&cap.export().unwrap()).unwrap());
        ...
        crate::CapCommand::List => match client.list_capabilities().await {
            Ok(Some(index)) => {
                ...
                for entry in index.entries {
                    ...
                    println!("  {} -> {} on {}", cap.issuer, cap.subject, cap.resource.canonical());
                    let mut perms = cap
                        .permissions
                        .iter()
                    ...
                    println!(
                        "  expires: {}",
                        cap.expires_at.map(|t| t.to_string()).unwrap_or_else(|| "never".into())
                    );
                }
            }
        ...
        crate::CapCommand::Revoke { id } => {
            match client.revoke_capability(&canopee_sdk::CapabilityId(id.clone())).await {
        ...
        crate::CapCommand::Check {
            ...
            if let Some(bundle_b64) = bundle {
                ...
                let exported: canopee_storage::ExportCapability =
                ...
                let subject = subject.unwrap_or_else(|| {
                    eprintln!("Error: pass a bundle, or --subject with --permission and --resource");
                    std::process::exit(1);
                });
